(function () {
    ...
    function applyCatalogQuickEdit() {
        ...
        toast("Ceny wstawione do JSON — kliknij „Zapisz ustawienia”.");
    }
    ...
    function bind() {
        ...
        $("settings-save-templates-btn")?.addEventListener("click", async () => {
            const text = $("settings-admin-templates")?.value || "";
            try {
                const ok = await window.CAD_Admin?.saveReplyTemplatesFromText?.(text);
                if (ok) {
                    window.CAD_Admin?.renderAdminTemplates?.();
                    toast("Szablony zapisane w chmurze (ten sam na telefonie i PC).");
                } else {
                    toast("Dodaj co najmniej jedną linię szablonu.", true);
                }
            } catch (e) {
