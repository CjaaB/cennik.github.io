(function () {
    ...
    async function refreshAdminInstallUi() {
        ...
        if (isInAppBrowser()) {
            ...
            setInstallStatus("Skopiuj link i otwórz w Chrome lub Safari.");
            return;
        }
        ...
        if (isIos()) {
            ...
            setInstallStatus("Wykonaj kroki poniżej — ikona CAD Admin na ekranie głównym.");
            return;
        }
        ...
        if (deferredInstallPrompt) {
            setInstallStatus("Stuknij „Zainstaluj teraz”, potem potwierdź na ekranie głównym.");
            if (pwaBtn) {
    ...
    function updateThreadHeader() {
        ...
        else parts.push("Brak e-mail — słaba identyfikacja");
    ...
    async function deleteMessage(msgId) {
        ...
        if (!isEmailAdmin(auth?.currentUser)) {
            toast("Usuwanie wymaga logowania e-mailem.", true);
            return;
        }
        ...
        try {
            ...
            toast(
                denied
                    ? "Brak uprawnień. Firebase → Rules → Publish (admin: delete na messages)."
    ...
    async function init() {
        ...
        if (location.protocol === "file:") {
            toast("Na PC otwórz panel przez https://cjaab.github.io/cennik.github.io/app/admin/ — nie z pliku na dysku.", true);
        }
