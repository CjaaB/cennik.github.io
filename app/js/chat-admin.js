(function () {
    ...
    function updateAdminStats() {
        const banner = $("admin-stats");
        const countEl = $("admin-stats-count");
        const inboxCount = $("admin-inbox-count");
        if (!countEl) return;
        ...
        if (banner) banner.hidden = unread === 0;

        if (inboxCount) {
            const n = visibleConversations().length;
            inboxCount.textContent = n ? `${n} na liście` : "";
        }

        window.CAD_AdminPanel?.updateNavBadges?.(unread);
        window.CAD_AdminPanel?.refreshStats?.();
    }
    ...
    function bindUi() {
        ...
        $("conv-lang-select")?.addEventListener("change", (e) => {
            listLangFilter = e.target.value || "";
            renderConvList();
        });
        $("admin-stats-goto")?.addEventListener("click", () => {
            listTab = "reply";
            document.querySelectorAll(".conv-tab").forEach((t) => {
                t.classList.toggle("active", t.dataset.tab === "reply");
            });
            renderConvList();
        });
    ...
    async function onAuthState(user) {
        ...
        $("admin-user-label").dataset.adminEmail = user.email || "";
