(function () {
    ...
    const LOCALE_KEY = window.CAD_CONFIG?.storageKeys?.locale || "cad_locale_v1";

    function readStoredLocale() {
        const saved = localStorage.getItem(LOCALE_KEY);
        if (["pl", "nl", "en"].includes(saved)) return saved;
        const legacy = localStorage.getItem("cad_app_locale");
        if (["pl", "nl", "en"].includes(legacy)) return legacy;
        return "pl";
    }
    ...
    function setLocale(lang) {
        ...
        window.CAD_LangSwitch?.syncAll?.(next);
        applyAppI18n();
    ...
    function readQuoteStateForBook() {
        try {
            const live = window.CAD_AppQuote?.getState?.();
            if (!live?.selectedServiceIds?.length) return null;
            const labels = live.selectedServiceIds.map((id) => {
                const item = window.CAD_DATA?.findCatalogItem?.(id);
                return item ? window.CAD_DATA.L(item.name) : id;
            });
            return { ...live, serviceLabels: labels };
        } catch (e) {
            return null;
        }
    }

    function syncBookExternalLink() {
        const btn = document.getElementById("book-external-btn");
        const frame = document.getElementById("book-frame");
        if (!btn || !frame) return;
        btn.href = frame.src || frame.getAttribute("data-base-url") || "#";
    }
    ...
    function showAppToast(message) {
        ...
        if (!el) {
            ...
            document.getElementById("app-root")?.prepend(el);
        }
        el.textContent = message;
        el.classList.add("show");
        window.clearTimeout(showAppToast._t);
        showAppToast._t = window.setTimeout(() => el.classList.remove("show"), 2800);
    }

    async function checkAppUpdateAvailable() {
        const banner = document.getElementById("app-update-banner");
        if (!banner || !window.CAD_VERSION?.client) return;
        try {
            const res = await fetch(`./sw.js?cad-check=${Date.now()}`, { cache: "no-store" });
            const text = await res.text();
            const m = text.match(/cad-app-v(\d+)/);
            const remote = m ? `cad-app-v${m[1]}` : "";
            if (!remote || remote === window.CAD_VERSION.client) {
                banner.hidden = true;
                return;
            }
            if (localStorage.getItem(APP_VER_DISMISS_KEY) === remote) {
                banner.hidden = true;
                return;
            }
    ...
    function bindAppUpdateBanner() {
        document.getElementById("app-update-reload")?.addEventListener("click", () => location.reload());
        document.getElementById("app-update-dismiss")?.addEventListener("click", () => {
            const banner = document.getElementById("app-update-banner");
            if (banner?.dataset.remoteVer) {
                localStorage.setItem(APP_VER_DISMISS_KEY, banner.dataset.remoteVer);
            }
            banner.hidden = true;
        });
    ...
    function showPanel(name) {
        if (!["home", "quote", "chat", "book", "more", "gallery", "location"].includes(name)) name = "home";
        window.CAD_APP.panel = name;
        document.querySelectorAll(".app-panel").forEach((p) => {
            p.classList.toggle("active", p.dataset.panel === name);
        });
        setBookFrameActive(name === "book");
        document.body.classList.toggle("cad-at-home", name === "home");
        document.body.classList.toggle("cad-in-section", name !== "home");
        document.body.classList.toggle("cad-panel-chat", name === "chat");
        document.body.classList.toggle("cad-panel-quote", name === "quote");
        document.body.classList.toggle("cad-panel-book", name === "book");
        if (name === "book") {
            updateBookFrameUrl();
            syncBookExternalLink();
        }
        if (name === "gallery") window.CAD_AppGallery?.refresh?.();
        if (name === "gallery" || name === "home" || name === "quote" || name === "chat" || name === "more" || name === "book" || name === "location") {
    ...
    function applyTheme(theme) {
        ...
        if (themeBtn && !window.CAD_Icons?.set(themeBtn, isLight ? "moon" : "sun")) {
    ...
    async function shareApp() {
        ...
        if (navigator.clipboard) {
            await navigator.clipboard.writeText(url);
            showAppToast(appT("quoteShareCopied") || "Link skopiowany.");
        }
    }
    ...
    function bindNav() {
        ...
        if (window.CAD_LangSwitch) {
            window.CAD_LangSwitch.mountInline(
                document.getElementById("app-lang-switch-root"),
                () => window.CAD_APP.locale,
                (next) => setLocale(next)
            );
        }
    ...
    function initFromHash() {
        ...
        if (["home", "quote", "chat", "book", "more", "gallery", "location"].includes(hash)) showPanel(hash);
        else showPanel("home");
    }
    ...
    window.CAD_AppShell = { openBookWithQuote, updateBookFrameUrl, showToast: showAppToast };
