(function () {
    ...
    const DEFAULT_CATALOG = [
        {
            ...
            items: [
                ...
                },
                {
                    ...
                    desc: { pl: "Odkurzanie, plastiki, szyby", nl: "Stofzuigen, plastiek, ramen", en: "Vacuum, plastics, glass" },
            ...
            items: [
                {
                    ...
                    desc: { pl: "Czyszczenie i odżywienie skóry", nl: "Reinigen en voeden van leer", en: "Clean and nourish leather" },
                    static: 70
                },
                {
                    id: "steering-wheel",
                ...
                },
                {
                    ...
                    desc: {
                        pl: "Osady drogowe, smoła, pył metaliczny",
                        nl: "Straatvuil, teer, metalen deeltjes",
                        en: "Road grime, tar, metal fallout"
                    },
                    static: 55
                },
                {
    ...
    const SETTINGS_CACHE_KEY = "cad_settings_cache_v2";
    ...
    const CATALOG_ICON_PATCH = {
        "int-basic": "fa-spray-can",
        "leather-clean": "fa-hand-sparkles",
        "steering-wheel": "fa-compact-disc",
        "boneting-seats": "fa-spray-can",
        "boneting-full": "fa-couch",
        "clay-deiron": "fa-wand-magic-sparkles"
    };

    const CATALOG_DESC_PATCH = {
        "clay-deiron": {
            pl: "Osady drogowe, smoła, pył metaliczny",
            nl: "Straatvuil, teer, metalen deeltjes",
            en: "Road grime, tar, metal fallout"
        ...
        "leather-clean": {
            pl: "Czyszczenie i odżywienie skóry",
            nl: "Reinigen en voeden van leer",
            en: "Clean and nourish leather"
        }
    };

    function normalizeLabelField(value, fallback) {
        if (value && typeof value === "object" && !Array.isArray(value)) {
            const pl = value.pl || value.nl || value.en || "";
            return {
                pl: value.pl || pl,
                nl: value.nl || fallback?.nl || pl,
                en: value.en || fallback?.en || pl
            };
        }
        const text = String(value || fallback?.pl || "");
        return {
            pl: text,
            nl: fallback?.nl || text,
            en: fallback?.en || text
    ...
    function defaultCatalogItem(id) {
        return findCatalogItem(id, DEFAULT_CATALOG);
    }

    function patchCatalogItems(catalog) {
        if (!Array.isArray(catalog)) return;
        catalog.forEach((group) => {
            const defGroup = DEFAULT_CATALOG.find(
                (g) => g.cat?.pl && group.cat?.pl && g.cat.pl === group.cat.pl
            ) || DEFAULT_CATALOG.find((g) =>
                (g.items || []).some((it) => (group.items || []).some((gi) => gi.id === it.id))
            );
            group.cat = normalizeLabelField(group.cat, defGroup?.cat);
            (group.items || []).forEach((item) => {
                if (!item?.id) return;
                const def = defaultCatalogItem(item.id);
                if (def?.prices) {
                    item.prices = { ...def.prices, ...(item.prices || {}) };
                }
                if (def?.static != null && item.static == null) {
                    item.static = def.static;
                }
                if (CATALOG_ICON_PATCH[item.id]) item.icon = CATALOG_ICON_PATCH[item.id];
                if (CATALOG_DESC_PATCH[item.id]) item.desc = { ...CATALOG_DESC_PATCH[item.id] };
                item.name = normalizeLabelField(item.name, def?.name);
                item.desc = normalizeLabelField(item.desc, def?.desc || CATALOG_DESC_PATCH[item.id]);
            });
        });
    }
    ...
    function mergeSettings(remote) {
        ...
        if (Array.isArray(remote.adminReplyTemplates) && remote.adminReplyTemplates.length) {
            base.adminReplyTemplates = remote.adminReplyTemplates
                .map((s) => String(s).trim())
                .filter(Boolean)
                .slice(0, 12);
        }
        patchCatalogItems(base.catalog);
        return base;
    }
    ...
    function convStatusLabel(data) {
        ...
        if (data.adminHandled && data.lastSender === "client") {
            return { key: "handled", pl: "Załatwione", nl: "Afgehandeld", en: "Handled" };
        }
    ...
    function statusClass(key) {
        ...
        if (key === "handled") return "status-handled";
        return "status-reply";
    }
    ...
    function L(obj, lang) {
        ...
        return obj[code] || obj.pl || obj.nl || obj.en || "";
    }
    ...
    function findDefaultCatalogItem(id) {
        return findCatalogItem(id, DEFAULT_CATALOG);
    }
