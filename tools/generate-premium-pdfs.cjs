...
async function createAssets(pdfLib, doc) {
    ...
    const logoBytes = Uint8Array.from(fs.readFileSync(path.join(ROOT, "assets", "logo1.png"))).buffer;
    return {
...
(async () => {
    ...
    fs.writeFileSync(path.join(ROOT, "assets", "protokol-wzor.pdf"), await buildProtocol(pdfLib, fontkit));
    fs.writeFileSync(path.join(ROOT, "assets", "regulamin.pdf"), await buildTerms(pdfLib, fontkit));
    console.log("Generated premium PDF files in assets/: protokol-wzor.pdf, regulamin.pdf");
})();
