...
function generateOfferPDF() {
    ...
    let content = `
        ...
        <body>
            ...
            <h2 class="document-title">Oferta dla klienta</h2>
            
            <div class="info-box">
                <div class="info-grid">
                    <p><strong>Data:</strong> ${new Date().toLocaleDateString()}</p>
                    <p><strong>Wielkość pojazdu:</strong> ${selectedSize}</p>
                    ${selectedCarModel ? `<p><strong>Model auta:</strong> ${selectedCarModel}</p>` : "<p><strong>Model auta:</strong> do uzupełnienia</p>"}
                    <p><strong>Status:</strong> wycena orientacyjna</p>
                </div>
            </div>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderWeatherForecast(forecast, daysContainer, statusEl, mainIcon, locationName) {
        ...
        daysContainer.innerHTML = assessed.map((day) => `
            <article class="weather-day weather-day--${day.tone}">
                ...
                <em>Deszcz ${day.rainChance}% • wiatr ${day.wind} km/h</em>
            </article>
        `).join("");
    }

    function renderWeatherFallback(daysContainer, statusEl, mainIcon) {
        statusEl.textContent = "Nie udało się pobrać prognozy. Termin usług zewnętrznych najlepiej potwierdzić w wiadomości.";
        if (mainIcon) mainIcon.innerHTML = '<i class="fas fa-cloud" aria-hidden="true"></i>';
        daysContainer.innerHTML = `
            <article class="weather-day weather-day--caution">
                <span>Prognoza</span>
                <strong>--°</strong>
                <small>Chwilowo niedostępna</small>
                <em>Ustalimy warunki przed usługą</em>
            </article>
        `;
    }
    ...
    function assessDetailingWeather(day) {
        ...
        if (stormOrSnow || day.rainChance >= 70 || day.wind >= 42 || day.tempMax <= 4) {
            return {
                tone: "bad",
                label: "Lepiej dogadać godzinę"
            };
        }
        if (rainyCode || day.rainChance >= 40 || day.wind >= 30 || day.tempMax <= 8) {
            return {
                tone: "caution",
                label: "Może być lepszy dzień na wnętrze"
            };
        }
        return {
            tone: "good",
            label: "Dobre warunki na zewnętrze"
        };
    }

    function buildWeatherSummary(day, cautionDays, locationName) {
        if (day.tone === "good" && cautionDays === 0) {
            return `${locationName}: najbliższe dni wyglądają dobrze pod mycie zewnętrzne.`;
        }
        if (day.tone === "bad") {
            return `${locationName}: dziś warunki mogą utrudniać mycie zewnętrzne, warto dogadać termin.`;
        }
        return `${locationName}: pogoda jest zmienna, więc usługi zewnętrzne mogą wymagać ustalenia godziny.`;
    }

    function formatWeatherDay(dateString, index) {
        if (index === 0) return "Dziś";
        if (index === 1) return "Jutro";
        return new Intl.DateTimeFormat("pl-PL", { weekday: "short" }).format(new Date(dateString));
    }
