...
function generateOfferPDF() {
    ...
    let content = `
        ...
        <body>
            <div class="page">
            <div class="header">
                <img src="${document.body.classList.contains("cad-app-quote") ? "../assets/logo1.png" : "assets/logo1.png"}" alt="Car All Detailing">
...
document.addEventListener("DOMContentLoaded", () => {
        ...
        document.querySelectorAll(".service-item").forEach((card) => {
            if (!isCoarse) {
                ...
                card.addEventListener("click", (e) => {
                    if (e.target.closest(".service-preview-close")) return;
                    if (card.classList.contains("service-preview-open")) {
                        closePreview(card);
                        const input = card.querySelector("input.service");
    ...
    function updateWeatherBubble(mainDay, locationName) {
        ...
        const iconWrap = bubble.querySelector(".weather-bubble__icon");
        if (iconWrap) {
            iconWrap.innerHTML = `<i class="${getWeatherIcon(mainDay.code)}" aria-hidden="true"></i>`;
        }

        const title = document.getElementById("weather-bubble-title");
        if (title) title.textContent = cadT("weatherTitle");

        const meta = document.getElementById("weather-bubble-meta");
        if (meta) {
            meta.textContent = `${locationName} · ${mainDay.tempMax}° · ${getWeatherToneLabel(mainDay.tone)}`;
        }
    ...
    function syncWeatherViews(assessed, locationName, mainDay) {
        ...
        if (modalDays) modalDays.innerHTML = daysHtml;
    ...
    function setupWeatherToggle(widget) {
        ...
        document.addEventListener("keydown", (event) => {
            ...
            if (!isWeatherDesktop() && event.key === "Escape") setInlineOpen(false);
        });
    ...
    function renderWeatherFallback(daysContainer, statusEl, mainIcon) {
        ...
        const modalDays = document.getElementById("weather-modal-days");
        const modalSubtitle = document.getElementById("weather-modal-subtitle");
        if (modalDays) modalDays.innerHTML = fallbackHtml;
