...
## Krok 6 — Firebase Storage (opcjonalnie — zdjęcia w czacie)

**Domyślnie wyłączone** (`chatPhotosEnabled: false` w `firebase-config.js`). Czat tekstowy, szablony, wycena i admin działają **bez Storage**.

Firebase często wymaga podpięcia karty przy włączeniu Storage — jeśli tego nie robisz, **pomiń ten krok**.

Gdy kiedyś włączysz Storage:

1. **Build** → **Storage** → **Get started**  
2. **Rules** → wklej treść z **`firebase/storage.rules`** → **Publish**  
3. W `firebase-config.js` ustaw **`chatPhotosEnabled: true`**
